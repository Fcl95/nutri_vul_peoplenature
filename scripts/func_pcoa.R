...
cwm_fren <- functcomp(food_source_nutri, as.matrix(cons_food_month))  
head(cwm_fren)
...
poca_muni <- ggplot(pcoa_eixos, aes(x = Axis.1, y = Axis.2)) +
  ...
  annotate('text', x = (pca_load$Axis.1*6), y = (pca_load$Axis.2*5.2),
...
polygon2 <- Polygon(pca_hull_sj[,2:3])
area2 <- polygon2@area
print(area2)
