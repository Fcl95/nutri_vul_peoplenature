...
RI <- traits
RI<-RI["top_nutri"] #0ou trocar por f_ori
ERS <- cbind(Axes, RI)
NoS <- 116

# Cenário de perda das espécies mais raras
Simulat_rare <- ERS[order(ERS[,"top_nutri"]),]
...
Simulat_comm <- ERS[order(ERS[,"top_nutri"], decreasing = TRUE),]
...
for (j in 1:1000) {
  dataperm <- ERS
  dataperm[,"top_nutri"] <- sample(dataperm[,"top_nutri"])
  Null_Simulat <- dataperm[order(dataperm[,"top_nutri"]),]
