...
Simulat_rare <- ERS[order(ERS[,"Vulnerability"]),]
...
Simulat_comm <- ERS[order(ERS[,"Vulnerability"], decreasing = TRUE),]
...
for (j in 1:1000) {
  dataperm <- ERS
  dataperm[,"Vulnerability"] <- sample(dataperm[,"Vulnerability"])
  Null_Simulat <- dataperm[order(dataperm[,"Vulnerability"]),]
...
# Plotar o gráfico
(vuln_plot <- ggplot(df_long, aes(x = Spp_erosion, y = FRic, color = scenario, linetype = scenario)) +
    ...
    labs(x = "Species erosion (%)", y = "NRic", color = "Scenario",
         linetype = "Scenario", 
         title = "Vulnerability") +
