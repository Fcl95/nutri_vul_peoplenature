...
Simulat_rare <- ERS[order(ERS[,"Trophic_level"]),]
...
Simulat_comm <- ERS[order(ERS[,"Trophic_level"], decreasing = TRUE),]
...
for (j in 1:1000) {
  dataperm <- ERS
  dataperm[,"Trophic_level"] <- sample(dataperm[,"Trophic_level"])
  Null_Simulat <- dataperm[order(dataperm[,"Trophic_level"]),]
...
# Plotar o gráfico
(troph_plot <- ggplot(df_long, aes(x = Spp_erosion, y = FRic, color = scenario, linetype = scenario)) +
  ...
  labs(x = " ", y = " ", color = "Scenario", linetype = "Scenario", 
       title = "Trophic Level") +
